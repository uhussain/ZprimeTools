...
void ZprimeJetsClass_MC_ZJets::Loop(Long64_t maxEvents, int reportEvery)
{
  ...
  for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++) {
				...
				if(dPhiJetMETcut(jetveto,METPhi_to_use))
				  {
				    ...
				    if (Pt123Fraction_to_use[0] > 0.85) fillHistos(jetCand,9,event_weight);
				  }
			      }   
			  }	
		      }
    ...
    vector<double> jetPtCut = {100,120,150,180,200,225,250,275,300,325,350,375,400,425,450,475,500};
    
    for (int i = 0; i < jetPtCut.size(); i++)
      { 
	jetCand.clear();
	j1PFConsPt.clear();
	j1PFConsEta.clear();
	...
	jetCand = getJetCand(jetveto,jetPtCut[i],2.4,0.8,0.1,0);
	AllPFCand(jetCand,PFCandidates);
	getPt123Frac(0);
	if (metFilters==0)
	  {
	    if (true)//Mono-jet triggers
	      {
		if(jetCand.size()>0)
		  {
		    if (pfMET>250)
				...
				if(btagVeto(0))
				  {
				    ...
				    if(dPhiJetMETcut(jetveto,METPhi_to_use))
				      {
					fillHistos(jetCand,10+2*i,event_weight);

					if (Pt123Fraction_to_use[0] > 0.85) fillHistos(jetCand,11+2*i,event_weight);
...
void ZprimeJetsClass_MC_ZJets::BookHistos(const char* file2)
{
  ...
  double PtBins[53]={100.,120.,150.,180.,200.,220.,240.,260.,280.,300.,320.,340.,360.,380.,400.,420.,440.,460.,480.,500.,520.,540.,560.,580.,600.,620.,640.,660.,680.,700.,720.,740.,760.,
  ...
  for(int i=0; i<44; i++){
    ...
    h_j1Pt[i]  = new TH1F(("j1pT"+histname).c_str(), "j1pT;p_{T} of Leading Jet (GeV)", 52,PtBins);h_j1Pt[i]->Sumw2();
