...
std::vector<std::pair<int,double>> ZprimeJetsClass_MC::getJetCand(double jetPtCut, double jetEtaCut, double jetNHFCut, double jetCHFCut){
...
std::vector<int> ZprimeJetsClass_MC::JetVetoDecision(int leading_lep_index, int subleading_lep_index) {
...
std::vector<int> ZprimeJetsClass_MC::electron_veto_looseID(int jet_index, int leading_mu_index, int subleading_mu_index, float elePtCut)
{
  std::vector<int> ele_cands;
  ele_cands.clear();
  ...
  for(int i = 0; i < nEle; i++)
      ...
      if(pass_SigmaIEtaIEtaFull5x5 && pass_dEtaIn && pass_dPhiIn && pass_HoverE && pass_iso && pass_ooEmooP && pass_d0 && pass_dz && pass_missingHits && pass_convVeto)
	{
	  ...
	  if(elePt->at(i) > elePtCut)
	    {
	      //Electron does not overlap jet and muons
...
std::vector<int> ZprimeJetsClass_MC::muon_veto_looseID(int jet_index, int leading_ele_index, int subleading_ele_index, float muPtCut)
{
  std::vector<int> mu_cands;
  mu_cands.clear();
