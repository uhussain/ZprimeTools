  ...
  for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++) {
	...
	if ((HLTEleMuX>>4&1 == 1) || (HLTEleMuX>>38&1 == 1)) //"HLT_Ele27_WPTight_Gsf_v or HLT_Ele115_CaloIdVT_GsfTrkIdT_v"
	  //if (HLTJet>>8&1 == 1) //"HLT_PFMET170_HBHECleaned_v"
	  {
		...
		std::vector<int> elelist = electron_veto_looseID(jetCand[0],0,10.0);
		std::vector<int> mulist;
		mulist.clear();   
		if(elelist.size() == 1)
                  {
		    lepindex = elelist[0];
		    mulist = muon_veto_looseID(jetCand1[0],elelist[0],10.0);
		    jetveto = JetVetoDecision(jetCand1[0],elelist[0]);
...
void ZprimeJetsClass::fillHistos(int histoNumber)
{
  ...
  if(lepton_pt > 0 && lepton_mass > 0){
    ...
    h_leptonPt[histoNumber]->Fill(lepton_pt);
    h_leptonM[histoNumber]->Fill(lepton_mass);}
}
...
std::vector<int> ZprimeJetsClass::JetVetoDecision(int jet_index, int ele_index) {
