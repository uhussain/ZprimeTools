...
#include "postAnalyzer_WlnG_mc_znng_PDFscale.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include "TH1F.h"
                ...
                if(looseMus.size() == 0)
                {
                  if(pfMET > 50)
                  {
                    ...
                    if(lepMET_MT < 160)
                    {
                      ...
                      //Scale variations
                      fillHistos(nMCreplicas,event_weight*genWeight_QCDscale_muR1_muF1,phoCand1[0],jetveto,elelist,leptoMET,leptoMET_phi); //Default
                      fillHistos(nMCreplicas+1,event_weight*genWeight_QCDscale_muR1_muF2,phoCand1[0],jetveto,elelist,leptoMET,leptoMET_phi); //Fac. up
                      fillHistos(nMCreplicas+2,event_weight*genWeight_QCDscale_muR1_muF0p5,phoCand1[0],jetveto,elelist,leptoMET,leptoMET_phi); //Fac. down
                      fillHistos(nMCreplicas+3,event_weight*genWeight_QCDscale_muR2_muF1,phoCand1[0],jetveto,elelist,leptoMET,leptoMET_phi); //Ren. up
                      fillHistos(nMCreplicas+4,event_weight*genWeight_QCDscale_muR2_muF2,phoCand1[0],jetveto,elelist,leptoMET,leptoMET_phi); //Both up
                      fillHistos(nMCreplicas+5,event_weight*genWeight_QCDscale_muR2_muF0p5,phoCand1[0],jetveto,elelist,leptoMET,leptoMET_phi);
                      fillHistos(nMCreplicas+6,event_weight*genWeight_QCDscale_muR0p5_muF1,phoCand1[0],jetveto,elelist,leptoMET,leptoMET_phi); //Ren. down
                      fillHistos(nMCreplicas+7,event_weight*genWeight_QCDscale_muR0p5_muF2,phoCand1[0],jetveto,elelist,leptoMET,leptoMET_phi);
                      fillHistos(nMCreplicas+8,event_weight*genWeight_QCDscale_muR0p5_muF0p5,phoCand1[0],jetveto,elelist,leptoMET,leptoMET_phi); //Both down
                    }
                  }
                }
              }
  ...
  for(int i = 0; i < nJet; i++)
    {
      ...
      if(deltar>0.4 && deltar_ele > 0.4 && jetPt->at(i) >30.0 && jetPFLooseId->at(i)==1 && jetPUID->at(i)>value)
        {
