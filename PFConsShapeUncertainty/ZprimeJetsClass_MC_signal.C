...
void ZprimeJetsClass_MC_signal::Loop(Long64_t maxEvents, int reportEvery)
{
		...
		if (pfMET>250)
		  {
				...
				if(dPhiJetMETcut(jetveto,METPhi_to_use))
				  {
				    ...
				    if (Pt123Fraction_to_use[0]>0.6)
				      {
            //std::cout<<run<<":"<<lumis<<":"<<event<<std::endl;
            //int TrkUncEntries,EcalUncEntries,HcalUncEntries;
            //TrkUncEntries=EcalUncEntries=HcalUncEntries=0;
				    //for(int i=0;i<TrackerCand.size();i++){
            //  if(j1PFConsPt.at(TrackerCand[i])>1.0){ 
            //    TrkUncEntries+=1;
            ////    std::cout<<"ChConsPt: "<<j1PFConsPt.at(TrackerCand[i])<<" Unc: "<<j1PFConsPtUnc.at(TrackerCand[i])<<std::endl;
            //}} 
				    //for(int i=0;i<EcalCand.size();i++){
            //  if(j1PFConsPt.at(EcalCand[i])>1.0){ 
            //    EcalUncEntries+=1;
            ////    std::cout<<"PhotonConsPt: "<<j1PFConsPt.at(EcalCand[i])<<" Unc: "<<j1PFConsPtUnc.at(EcalCand[i])<<std::endl;
            //}} 
				    //for(int i=0;i<HcalCand.size();i++){
            //  if(j1PFConsPt.at(HcalCand[i])>1.0){ 
            //    HcalUncEntries+=1;
            ////    std::cout<<"NeutHadronConsPt: "<<j1PFConsPt.at(HcalCand[i])<<" Unc: "<<j1PFConsPtUnc.at(HcalCand[i])<<std::endl;
            //    }}
            //TrackerCount+=TrkUncEntries;
            //EcalCount+=EcalUncEntries;
            //HcalCount+=HcalUncEntries;
            //std::cout<<"AllNormPt123Fraction: "<<Pt123Fraction_to_use[0]<<std::endl;
            //std::cout<<"TrkNormPt123Fraction: "<<Pt123Fraction_to_use[1]<<std::endl;
            //std::cout<<"EcalNormPt123Fraction: "<<Pt123Fraction_to_use[2]<<std::endl;
            //std::cout<<"HcalNormPt123Fraction: "<<Pt123Fraction_to_use[3]<<std::endl;
            //std::cout<<"NormPt123FractionVectorSize: "<<Pt123Fraction_to_use.size()<<std::endl;
					fillHistos(jetCand,9,event_weight);
				      }
				    if (Pt123Fraction_to_use[0]>0.7)
				      {
            ...
            //0=AllUncPt123Fraction,1=TrkUncOnly,2=EcalUncOnly,3=HcalUncOnly
            Pt123Fraction_to_use=getPt123Frac(1);
				    PtFraction_to_use=getPtFrac(1);
            //PFUncUp for Basic selections to compare to fillhistos(8)
            if(true){
              fillHistos(jetCand,14,event_weight);
              }
            //TrackerUncUp
				    if (Pt123Fraction_to_use[1]>0.6)
				      {
					fillHistos(jetCand,15,event_weight);
				      }
            //EcalUncUp
				    if (Pt123Fraction_to_use[2]>0.6)
				      {
					fillHistos(jetCand,16,event_weight);
				      }
            //HcalUncUp
				    if (Pt123Fraction_to_use[3]>0.6)
				      {
					fillHistos(jetCand,17,event_weight);
				      }
            //TrackerUncUp
				    if (Pt123Fraction_to_use[1]>0.7)
				      {
					fillHistos(jetCand,18,event_weight);
				      }
            //EcalUncUp
				    if (Pt123Fraction_to_use[2]>0.7)
				      {
					fillHistos(jetCand,19,event_weight);
				      }
            //HcalUncUp
				    if (Pt123Fraction_to_use[3]>0.7)
				      {
					fillHistos(jetCand,20,event_weight);
				      }
            //TrackerUncUp
				    if (Pt123Fraction_to_use[1]>0.75)
				      {
					fillHistos(jetCand,21,event_weight);
				      }
            //EcalUncUp
				    if (Pt123Fraction_to_use[2]>0.75)
				      {
					fillHistos(jetCand,22,event_weight);
				      }
            //HcalUncUp
				    if (Pt123Fraction_to_use[3]>0.75)
				      {
            ...
            Pt123Fraction_to_use=getPt123Frac(-1);
				    PtFraction_to_use=getPtFrac(-1);
            //PFUncDown for Basic selections to compare to fillhistos(8)
            if(true){
            ...
            //HcalUncDown
				    if (Pt123Fraction_to_use[3]>0.85)
				      {
					fillHistos(jetCand,45,event_weight);
				      }
				  }
			      }   
			  }	
  ...
  //std::cout<<"TrackerUncEntries_8: "<<TrackerCount<<std::endl;
  //std::cout<<"EcalUncEntries_8: "<<EcalCount<<std::endl;
  //std::cout<<"HcalUncEntries_8: "<<HcalCount<<std::endl;
...
std::vector<double>ZprimeJetsClass_MC_signal::getPt123Frac(int UncType)
{
...
std::vector<double>ZprimeJetsClass_MC_signal::getPtFrac(int UncType)
{
...
void ZprimeJetsClass_MC_signal::AllPFCand(std::vector<std::pair<int,double>> jetCand, std::vector<int> PFCandidates)
{
...
std::vector<int> ZprimeJetsClass_MC_signal::JetVetoDecision(int UncType) {
...
std::vector<int>ZprimeJetsClass_MC_signal::getPFCandidates(){
...
bool ZprimeJetsClass_MC_signal::btagVeto(int UncType) {
...
bool ZprimeJetsClass_MC_signal::dPhiJetMETcut(std::vector<int> jets, float METPhi)
{
