                                          ...
                                          if(DeltaPhi(jetPhi->at(jetveto[j]),pfMETPhi) < minDPhiJetMET)
                                            {
                                              ...
                                              if(j < 4){
                                                minDPhiJetMET_first4 = DeltaPhi(jetPhi->at(jetveto[j]),pfMETPhi);}
                                                      ...
                                                      //Start with FillHistos(16)
                                                      //nMCreplicas=101
                                                      for(unsigned int i = 0; i < nMCreplicas; i++)
                                                      {
                                                        fillHistos(i+16,event_weight*lheNormalizedWeights->at(vecIndices[i]));
                                                      }
                                                      //Scale variations
                                                      fillHistos(nMCreplicas+16,event_weight*genWeight_QCDscale_muR1_muF1); //Default
                                                      fillHistos(nMCreplicas+17,event_weight*genWeight_QCDscale_muR1_muF2); //Fac. up
                                                      fillHistos(nMCreplicas+18,event_weight*genWeight_QCDscale_muR1_muF0p5); //Fac. down
                                                      fillHistos(nMCreplicas+19,event_weight*genWeight_QCDscale_muR2_muF1); //Ren. up
                                                      fillHistos(nMCreplicas+20,event_weight*genWeight_QCDscale_muR2_muF2); //Both up
                                                      fillHistos(nMCreplicas+21,event_weight*genWeight_QCDscale_muR2_muF0p5);
                                                      fillHistos(nMCreplicas+22,event_weight*genWeight_QCDscale_muR0p5_muF1); //Ren. down
                                                      fillHistos(nMCreplicas+23,event_weight*genWeight_QCDscale_muR0p5_muF2);
                                                      fillHistos(nMCreplicas+24,event_weight*genWeight_QCDscale_muR0p5_muF0p5); //Both down
                                                      //fillHistos(101+24=125)
			      	                                                 }//closes dPhiJetMETcut(jetveto)
		      	                                                  }   
		                                                       }	
		                                                    }
	                                                    }
