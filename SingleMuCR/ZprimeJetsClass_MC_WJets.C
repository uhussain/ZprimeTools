	          ...
	          if(jetCand.size()>0)
	            {
	      	      ...
	      	      elelist.clear();
	      	      std::vector<int> mulist = muon_veto_tightID(jetCand[0].first,30.0);
	      	      std::vector<int> looseMus = muon_veto_looseID(jetCand[0].first,0,10.0);      
	      	      if(mulist.size() == 1 && looseMus.size() == 1)
                  { 
	      	          lepindex = mulist[0];
                    elelist = electron_veto_looseID(jetCand[0].first,lepindex,10.0);
                    jetveto = JetVetoDecision(jetCand[0].first,lepindex);
...
void ZprimeJetsClass_MC_WJets::AllPFCand(std::vector<pair<int,double>> jetCand,std::vector<int> PFCandidates)
{
...
std::vector<pair<int,double>> ZprimeJetsClass_MC_WJets::getJetCand(double jetPtCut, double jetEtaCut, double jetNHFCut, double jetCHFCut){
...
std::vector<int> ZprimeJetsClass_MC_WJets::JetVetoDecision(int jet_index, int mu_index) {
  ...
  for(int i = 0; i < nJet; i++)
    {
      ...
      if(deltar_mu>0.4 && deltar_jet>0.4 && jetPt->at(i) >30.0 && jetPFLooseId->at(i)==1)
        {
...
std::vector<int> ZprimeJetsClass_MC_WJets::electron_veto_looseID(int jet_index,int mu_index, float elePtCut)
{
  std::vector<int> ele_cands;
  ele_cands.clear();
...
std::vector<int> ZprimeJetsClass_MC_WJets::muon_veto_looseID(int jet_index, int ele_index, float muPtCut)
{
  std::vector<int> mu_cands;
  mu_cands.clear();
