				...
				if(dPhiJetMETcut(jetveto,METPhi_to_use))
				  {
				    ...
				    if (Pt123Fraction>0.6) fillHistos(jetCand,16);
				    if (Pt123Fraction>0.7) fillHistos(jetCand,17);
				    if (Pt123Fraction>0.75)fillHistos(jetCand,18);
				    if (Pt123Fraction>0.8) fillHistos(jetCand,19);
				    if (Pt123Fraction>0.85)fillHistos(jetCand,20);
				    if (Pt123Fraction>0.9) fillHistos(jetCand,21);
				  }
			      }   
			  }	
		      }
				...
				if(dPhiJetMETcut(jetveto,METPhi_to_use))
				  {
				    ...
				    if (Pt123Fraction>0.6) fillHistos(jetCand,24);
				    if (Pt123Fraction>0.7) fillHistos(jetCand,25);
				    if (Pt123Fraction>0.75)fillHistos(jetCand,26);
				    if (Pt123Fraction>0.8) fillHistos(jetCand,27);
				    if (Pt123Fraction>0.85)fillHistos(jetCand,28);
				    if (Pt123Fraction>0.9) fillHistos(jetCand,29);
				  }
			      }   
			  }	
		      }
			...
			if(electron_veto_looseID(jetCandDown[0].first,10) &&  muon_veto_looseID(jetCandDown[0].first,10))
			  {
				...
				if(dPhiJetMETcut(jetveto,METPhi_to_use))
				  {
				    fillHistos(jetCandDown,30);
				    
				    //This is for comparison with previous results (for all events)
				    if (jetetaWidth->at(jetCandDown[0].first)<0.04)
				      {
					fillHistos(jetCandDown,31);
				      }

				    if (Pt123Fraction>0.6) fillHistos(jetCand,32);
				    if (Pt123Fraction>0.7) fillHistos(jetCand,33);
				    if (Pt123Fraction>0.75)fillHistos(jetCand,34);
				    if (Pt123Fraction>0.8) fillHistos(jetCand,35);
				    if (Pt123Fraction>0.85)fillHistos(jetCand,36);
				    if (Pt123Fraction>0.9) fillHistos(jetCand,37);
				  }
			      }   
			  }	
		      }
